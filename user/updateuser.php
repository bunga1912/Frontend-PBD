...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($username) || empty($idrole)) {
            ...
            $stmt = $conn->prepare("SELECT COUNT(*) FROM user WHERE username = ? AND iduser != ?");
            ...
            if ($count > 0) {
                ...
                if (!empty($password)) {
                    ...
                    $stmt = $conn->prepare("UPDATE user SET username = ?, password = ?, idrole = ? WHERE iduser = ?");
                    $stmt->execute([$username, $password, $idrole, $id]);
                } else {
                    // Update tanpa mengubah password
                    $stmt = $conn->prepare("UPDATE user SET username = ?, idrole = ? WHERE iduser = ?");
                    $stmt->execute([$username, $idrole, $id]);
                }
...
<head>
    ...
    <title>Edit User - Mitra Jaya Supermarket</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="form-card">
            <h2 class="page-title">✏️ Edit Data User</h2>

            <div class="info-box">
                <strong>👥 ID User: <?php echo $user['iduser']; ?></strong>
                Mengubah data: <?php echo htmlspecialchars($user['username']); ?>
            </div>
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <div class="input-hint">Password minimal 6 karakter. Kosongkan jika tidak ingin mengubah password</div>
                </div>
                    ...
                    <select id="idrole" name="idrole" required>
                        ...
                        <?php foreach ($roles as $role): ?>
                            <option value="<?php echo $role['idrole']; ?>"
                                <?php echo ($role['idrole'] == $user['idrole']) ? 'selected' : ''; ?>>
                ...
                <div class="button-group">
                    ...
                    <button type="button" class="btn btn-secondary" onclick="window.location.href='indexuser.php'">
                        ↩️ Kembali
                    </button>
                </div>
            </form>
