...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM pengadaan WHERE idpengadaan = :id");
    $stmt->bindParam(':id', $id);
    $stmt->execute();
    $pengadaan = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$pengadaan) {
        $_SESSION['error'] = "Data pengadaan tidak ditemukan!";
        header("Location: indexpengadaan.php");
        exit();
    }
    
    // Ambil detail items
    $stmt = $conn->prepare("SELECT * FROM detail_pengadaan WHERE idpengadaan = :id");
    $stmt->bindParam(':id', $id);
    $stmt->execute();
    $detail_items = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Ambil data vendor aktif
    $stmt = $conn->query("SELECT idvendor, nama_vendor, badan_hukum FROM view_vendor_aktif ORDER BY nama_vendor ASC");
    ...
    // Ambil data user (purchasing & super admin)
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!isset($_POST['items']) || empty($_POST['items'])) {
            ...
            try {
                ...
                $sql = "UPDATE pengadaan SET ppn = :ppn, user_iduser = :iduser, 
                        status = :status, vendor_idvendor = :idvendor WHERE idpengadaan = :id";
                ...
                // Reset subtotal
                $stmt = $conn->prepare("UPDATE pengadaan SET subtotal_nilai = 0 WHERE idpengadaan = :id");
                $stmt->bindParam(':id', $id);
                $stmt->execute();
                ...
                foreach ($_POST['items'] as $item) {
                    if (!empty($item['idbarang']) && !empty($item['jumlah']) && $item['jumlah'] > 0) {
                        ...
                        $stmt_detail->bindParam(':idpengadaan', $id);
                        $stmt_detail->execute();
                    }
                }
                ...
                $_SESSION['success'] = "Data pengadaan berhasil diupdate!";
                header("Location: indexpengadaan.php");
                exit();
...
// Function untuk tampilan status
function getStatusLabel($status) {
    switch($status) {
        case 'P': return 'Pending (Menunggu Approval)';
        case 'A': return 'Approved (Disetujui)';
        case 'D': return 'Ditolak (Rejected)';
...
function getStatusBadge($status) {
    switch($status) {
        case 'P': return '<span style="background: #ffc107; color: #000; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;">⏳ Pending</span>';
        case 'A': return '<span style="background: #28a745; color: #fff; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;">✅ Approved</span>';
        case 'D': return '<span style="background: #dc3545; color: #fff; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;">❌ Ditolak</span>';
        default: return '<span style="background: #6c757d; color: #fff; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;">?</span>';
    }
}
...
<head>
    ...
    <title>Edit Pengadaan #<?php echo $id; ?> - Mitra Jaya Supermarket</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status-info { background: #e7f3ff; padding: 15px; border-radius: 10px; margin-bottom: 20px; border-left: 4px solid #2196F3; }
    ...
    <div class="main-content">
        ...
        <div class="content-card">
            ...
            <div class="status-info">
                <strong>ℹ️ Status Saat Ini:</strong> <?php echo getStatusBadge($pengadaan['status']); ?>
                <p style="margin-top: 8px; font-size: 13px; color: #666;">
                    <strong>Keterangan:</strong><br>
                    • <strong>Pending</strong> = Menunggu approval dari Super Admin<br>
                    • <strong>Approved</strong> = Disetujui, siap untuk penerimaan<br>
                    • <strong>Ditolak</strong> = Tidak disetujui, pengadaan dibatalkan
                </p>
            </div>
                ...
                <div class="form-row">
                        ...
                        <select name="iduser" required>
                            ...
                            <?php foreach ($users as $user): ?>
                                <option value="<?php echo $user['iduser']; ?>"
                                    <?php echo ($user['iduser'] == $pengadaan['user_iduser']) ? 'selected' : ''; ?>>
                        ...
                        <select name="idvendor" required>
                            ...
                            <?php foreach ($vendors as $vendor): ?>
                                <option value="<?php echo $vendor['idvendor']; ?>"
                                    <?php echo ($vendor['idvendor'] == $pengadaan['vendor_idvendor']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" required>
                            ...
                            <option value="P" <?php echo ($pengadaan['status'] == 'P') ? 'selected' : ''; ?>>
                                ⏳ Pending (Menunggu Approval)
                            </option>
                            <option value="A" <?php echo ($pengadaan['status'] == 'A') ? 'selected' : ''; ?>>
                                ✅ Approved (Disetujui)
                            </option>
                            <option value="D" <?php echo ($pengadaan['status'] == 'D') ? 'selected' : ''; ?>>
                                ❌ Ditolak (Rejected)
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        <label>PPN (Rp) *</label>
                        <input type="number" name="ppn" id="ppn" value="<?php echo $pengadaan['ppn']; ?>" min="0" required onchange="calculateTotal()">
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="number" name="items[<?php echo $index; ?>][harga_satuan]" 
                                       ...
                                       readonly style="background: #f8f9fa;" value="<?php echo $item['harga_satuan']; ?>">
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="text" class="subtotal-display" id="subtotal_<?php echo $index; ?>" 
                                       readonly style="background: #f8f9fa;" value="Rp <?php echo number_format($item['sub_total'], 0, ',', '.'); ?>">
                                <input type="hidden" class="subtotal-value" id="subtotal_val_<?php echo $index; ?>" value="<?php echo $item['sub_total']; ?>">
                            </td>
                            <td>
                                <button type="button" class="btn-remove" onclick="removeItem(this)">🗑️ Hapus</button>
                            </td>
                        </tr>
                        <?php 
                        $index++;
            ...
            row.innerHTML = `
                ...
                <td>
                    <input type="number" name="items[${itemCount}][harga_satuan]" 
                           class="harga-input" id="harga_${itemCount}" readonly 
                ...
                <td>
                    <input type="text" class="subtotal-display" id="subtotal_${itemCount}" 
                           readonly style="background: #f8f9fa;" value="Rp 0">
                    <input type="hidden" class="subtotal-value" id="subtotal_val_${itemCount}" value="0">
                </td>
                <td>
                    <button type="button" class="btn-remove" onclick="removeItem(this)">🗑️ Hapus</button>
                </td>
