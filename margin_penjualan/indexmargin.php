...
try {
    ...
    // Filter status - default tampilkan margin aktif
    $filter = isset($_GET['filter']) ? $_GET['filter'] : 'aktif';
    
    // Query berdasarkan filter dengan JOIN ke tabel user - PERBAIKAN DI SINI
    if ($filter == 'nonaktif') {
        $sql = "SELECT m.idmargin_penjualan, m.created_at, m.persen, m.status, 
                       m.iduser, m.updated_at, u.username
                FROM margin_penjualan m
                LEFT JOIN user u ON m.iduser = u.iduser
                WHERE m.status = 0 
                ORDER BY m.idmargin_penjualan DESC";
    } elseif ($filter == 'semua') {
        // PERBAIKAN: Query untuk menampilkan semua margin
        $sql = "SELECT m.idmargin_penjualan, m.created_at, m.persen, m.status, 
                       m.iduser, m.updated_at, u.username
                FROM margin_penjualan m
                LEFT JOIN user u ON m.iduser = u.iduser
        ...
        // Default: tampilkan margin aktif menggunakan VIEW
        $sql = "SELECT m.idmargin_penjualan, m.persen, m.iduser, u.username,
                       mp.created_at, mp.updated_at
                FROM view_margin_aktif m
                LEFT JOIN user u ON m.iduser = u.iduser
                LEFT JOIN margin_penjualan mp ON m.idmargin_penjualan = mp.idmargin_penjualan
                ORDER BY m.idmargin_penjualan DESC";
    }
...
<head>
    ...
    <title>Data Margin Penjualan - Mitra Jaya Supermarket</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-card">
            <h2 class="page-title">💹 Data Margin Penjualan</h2>
            ...
            <?php endif; ?>
            <div class="toolbar">
                <a href="createmargin.php" class="btn btn-primary">➕ Tambah Margin Penjualan</a>
                <div class="filter-dropdown">
                    <button class="filter-btn" onclick="toggleFilter()">
                        🔍 Filter: <?php 
                            if ($filter == 'nonaktif') echo 'Margin Nonaktif';
                            elseif ($filter == 'semua') echo 'Semua Margin';
                            else echo 'Margin Aktif';
                        ?> ▼
                    </button>
                    <div class="filter-menu" id="filterMenu">
                        <a href="indexmargin.php" class="<?php echo (!isset($_GET['filter']) || $filter == 'aktif') ? 'active' : ''; ?>">✅ Margin Aktif</a>
                        <a href="?filter=nonaktif" class="<?php echo $filter == 'nonaktif' ? 'active' : ''; ?>">❌ Margin Nonaktif</a>
                        <a href="?filter=semua" class="<?php echo $filter == 'semua' ? 'active' : ''; ?>">📋 Semua Margin</a>
                    </div>
                </div>
            </div>
            ...
            <?php if (empty($margins)): ?>
                <div class="empty-state"><div class="empty-state-icon">💹</div><h3>Tidak ada data margin penjualan</h3><p>Filter: <?php echo ucfirst($filter); ?></p><p>Silakan tambahkan margin penjualan baru atau ubah filter</p></div>
            <?php else: ?>
                <table>
                    <thead><tr><th>ID</th><th>Created At</th><th>Persen (%)</th><th>Status</th><th>User</th><th>Updated At</th><th>Aksi</th></tr></thead>
                    <tbody>
                        <?php foreach ($margins as $margin): ?>
                        <tr>
                            <td><?php echo $margin['idmargin_penjualan']; ?></td>
                            <td>
                                <span class="datetime-text">
                                    <?php 
                                        if (isset($margin['created_at']) && $margin['created_at']) {
                                            $date = new DateTime($margin['created_at']);
                            ...
                            <td><strong><?php echo number_format($margin['persen'], 2); ?>%</strong></td>
                            <td>
                                <span class="status-badge <?php echo (isset($margin['status']) && $margin['status'] == 1) || !isset($margin['status']) ? 'status-aktif' : 'status-nonaktif'; ?>">
                                    <?php echo (isset($margin['status']) && $margin['status'] == 1) || !isset($margin['status']) ? 'Aktif' : 'Nonaktif'; ?>
                                </span>
                            </td>
                            <td><?php echo htmlspecialchars($margin['username'] ?? '-'); ?></td>
                            <td>
                                <span class="datetime-text">
                                    <?php 
                                        if (isset($margin['updated_at']) && $margin['updated_at']) {
                                            $date = new DateTime($margin['updated_at']);
                                            echo $date->format('d/m/Y H:i');
                                        } else {
                                            echo '-';
                                        }
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-edit" onclick="window.location.href='updatemargin.php?id=<?php echo $margin['idmargin_penjualan']; ?>'">Edit</button>
                                    <button class="btn-delete" onclick="confirmDelete(<?php echo $margin['idmargin_penjualan']; ?>)">Delete</button>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
    ...
    <script>
        ...
        function confirmDelete(id) {
            if (confirm('Apakah Anda yakin ingin menghapus margin penjualan ini?')) {
                window.location.href = 'deletemargin.php?id=' + id;
            }
        }
    </script>
</body>
