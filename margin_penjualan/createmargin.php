...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $persen = $_POST['persen'];
        $status = $_POST['status'];
        $iduser = $_SESSION['iduser']; // Ambil dari session user yang login
        
        // Validasi
        if (empty($persen) || empty($status)) {
            $error = "Semua field harus diisi!";
        } elseif ($persen < 0 || $persen > 100) {
            $error = "Persentase harus antara 0 sampai 100!";
        } else {
            // Insert ke database (created_at dan updated_at otomatis)
            $stmt = $conn->prepare("INSERT INTO margin_penjualan (persen, status, iduser) VALUES (?, ?, ?)");
            $stmt->execute([$persen, $status, $iduser]);
            
            $success = "Margin penjualan berhasil ditambahkan!";
...
<head>
    ...
    <title>Tambah Margin Penjualan - Mitra Jaya Supermarket</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="form-card">
            <h2 class="page-title">💹 Tambah Margin Penjualan Baru</h2>
            <?php if ($success): ?>
            ...
            <form method="POST" action="">
                <div class="form-group">
                    ...
                    <input type="number" id="persen" name="persen" required placeholder="Contoh: 5.0, 10.5, 15.0" min="0" max="100" step="0.01">
                ...
                <div class="button-group">
                    ...
                    <button type="button" class="btn btn-secondary" onclick="window.location.href='indexmargin.php'">↩️ Kembali</button>
                </div>
            </form>
        </div>
    </div>
