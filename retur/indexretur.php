...
try {
    ...
    // Query untuk mendapatkan data retur dengan JOIN ke user dan penerimaan
    // Menghitung jumlah item dari detail_penerimaan (bukan detail_retur)
    $sql = "SELECT r.idretur, r.created_at, r.idpenerimaan, r.iduser,
               u.username,
               pen.idpengadaan,
               (SELECT COUNT(*) FROM detail_retur WHERE idretur = r.idretur) as jumlah_item
        FROM retur r
...
<head>
    ...
    <title>Data Retur - Mitra Jaya Supermarket</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge { padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; display: inline-block; background: #e3f2fd; color: #1976d2; }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-card">
            <h2 class="page-title">↩️ Daftar Retur Barang</h2>
            ...
            <div class="toolbar">
                <a href="createretur.php" class="btn btn-primary">➕ Tambah Retur Baru</a>
            </div>
            ...
            <?php if (empty($returs)): ?>
                <div class="empty-state">
                    ...
                    <h3>Tidak ada data retur</h3>
                    <p>Silakan tambahkan retur barang baru</p>
                </div>
            <?php else: ?>
                <table>
                    <thead>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><strong>#<?php echo $retur['idretur']; ?></strong></td>
                            <td>
                                <span class="datetime-text">
                                    <?php 
                                        if (isset($retur['created_at']) && $retur['created_at']) {
                                            $date = new DateTime($retur['created_at']);
                            ...
                            <td><span class="badge">#<?php echo $retur['idpenerimaan']; ?></span></td>
                            <td><span class="badge">#<?php echo $retur['idpengadaan'] ?? '-'; ?></span></td>
                            <td><?php echo htmlspecialchars($retur['username'] ?? '-'); ?></td>
                            <td><strong><?php echo $retur['jumlah_item']; ?> item</strong></td>
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-delete" onclick="confirmDelete(<?php echo $retur['idretur']; ?>)">Delete</button>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
    ...
    <script>
        function confirmDelete(id) {
            if (confirm('Apakah Anda yakin ingin menghapus data retur ini?')) {
                window.location.href = 'deleteretur.php?id=' + id;
            }
        }
    </script>
</body>
