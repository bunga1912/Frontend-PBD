...
try {
    ...
    // Filter status - default tampilkan vendor aktif
    $filter = isset($_GET['filter']) ? $_GET['filter'] : 'aktif';
    
    // Query berdasarkan filter menggunakan VIEW
    if ($filter == 'nonaktif') {
        $sql = "SELECT * FROM vendor WHERE status = 0 ORDER BY idvendor DESC";
    } elseif ($filter == 'semua') {
        $sql = "SELECT * FROM view_vendor ORDER BY idvendor DESC";
    } else {
        // Default: tampilkan vendor aktif menggunakan VIEW
        $sql = "SELECT * FROM view_vendor_aktif ORDER BY idvendor DESC";
    }
...
<head>
    ...
    <title>Data Vendor - Mitra Jaya Supermarket</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="main-content">
        <div class="header">
            ...
            <div class="header-actions">
                <button class="btn btn-logout" onclick="window.location.href='../login/logout.php'">
                    🚪 Logout
                </button>
            </div>
        </div>
        ...
        <div class="content-card">
            <h2 class="page-title">🏢 Daftar Vendor</h2>
            ...
            <div class="toolbar">
                <a href="createvendor.php" class="btn btn-primary">
                    ➕ Tambah Vendor
                </a>
                ...
                <div class="filter-dropdown">
                    <button class="filter-btn" onclick="toggleFilter()">
                        🔍 Filter: <?php 
                            if ($filter == 'nonaktif') echo 'Vendor Nonaktif';
                            elseif ($filter == 'semua') echo 'Semua Vendor';
                            else echo 'Vendor Aktif';
                        ?> ▼
                    </button>
                    <div class="filter-menu" id="filterMenu">
                        <a href="indexvendor.php" class="<?php echo (!isset($_GET['filter']) || $filter == 'aktif') ? 'active' : ''; ?>">
                            ✅ Vendor Aktif
                        </a>
                        <a href="?filter=nonaktif" class="<?php echo $filter == 'nonaktif' ? 'active' : ''; ?>">
                            ❌ Vendor Nonaktif
                        </a>
                        <a href="?filter=semua" class="<?php echo $filter == 'semua' ? 'active' : ''; ?>">
                            📋 Semua Vendor
                        </a>
                    </div>
                </div>
            ...
            <?php if (empty($vendors)): ?>
                <div class="empty-state">
                    ...
                    <h3>Tidak ada data vendor</h3>
                    <p>Filter: <?php echo ucfirst($filter); ?></p>
                    <p>Silakan tambahkan vendor baru atau ubah filter</p>
                </div>
            <?php else: ?>
                <table>
                    <thead>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?php echo $vendor['idvendor']; ?></td>
                            <td><strong><?php echo htmlspecialchars($vendor['nama_vendor']); ?></strong></td>
                            <td><?php echo $vendor['badan_hukum'] == 1 ? 'Ya' : 'Tidak'; ?></td>
                            <td>
                                <span class="status-badge <?php echo ($vendor['status'] ?? 1) == 1 ? 'status-aktif' : 'status-nonaktif'; ?>">
                                    <?php echo ($vendor['status'] ?? 1) == 1 ? 'Aktif' : 'Nonaktif'; ?>
                                </span>
                            </td>
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-edit" onclick="window.location.href='updatevendor.php?id=<?php echo $vendor['idvendor']; ?>'">
                                        Edit
                                    </button>
                                    <button class="btn-delete" onclick="confirmDelete(<?php echo $vendor['idvendor']; ?>)">
                                        Delete
                                    </button>
                                </div>
                            </td>
