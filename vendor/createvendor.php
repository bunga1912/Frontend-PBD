...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $status = 1; // Status selalu Aktif untuk vendor baru
        
        // PERBAIKAN: Validasi menggunakan isset() untuk badan_hukum
        // Karena empty(0) akan return true, padahal 0 adalah value yang valid
        if (empty($nama_vendor) || !isset($badan_hukum) || $badan_hukum === '') {
            $error = "Semua field harus diisi!";
        } else {
            // Insert ke database
            $stmt = $conn->prepare("INSERT INTO vendor (nama_vendor, badan_hukum, status) VALUES (?, ?, ?)");
            $stmt->execute([$nama_vendor, $badan_hukum, $status]);
...
<head>
    ...
    <title>Tambah Vendor - Mitra Jaya Supermarket</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="form-card">
            <h2 class="page-title">🏢 Tambah Vendor Baru</h2>
            <?php if ($success): ?>
            ...
            <form method="POST" action="">
                <div class="form-group">
                    ...
                    <input type="text" id="nama_vendor" name="nama_vendor" required placeholder="Contoh: PT Sumber Rejeki" maxlength="100">
                    <div class="input-hint">Masukkan nama lengkap vendor/supplier (maksimal 100 karakter)</div>
                </div>
                <div class="form-group">
                    ...
                    <select id="badan_hukum" name="badan_hukum" required>
                        ...
                        <option value="1">Ya (PT/CV)</option>
                        <option value="0">Tidak (UD/Perorangan)</option>
                    </select>
                    <div class="input-hint">Pilih apakah vendor memiliki badan hukum resmi</div>
                </div>
                <div class="button-group">
                    <button type="submit" class="btn btn-primary">💾 Simpan</button>
                    <button type="button" class="btn btn-secondary" onclick="window.location.href='indexvendor.php'">↩️ Kembali</button>
                </div>
            </form>
        </div>
    </div>
