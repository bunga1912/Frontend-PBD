...
try {
    ...
    // Query untuk mengambil data role menggunakan VIEW
    $sql = "SELECT * FROM view_role ORDER BY idrole DESC";
    $stmt = $conn->query($sql);
...
<head>
    ...
    <title>Data Role - Mitra Jaya Supermarket</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-card">
            <h2 class="page-title">🎭 Daftar Role</h2>
            ...
            <?php endif; ?>
            <div class="toolbar">
                <a href="createrole.php" class="btn btn-primary">➕ Tambah Role</a>
            </div>
            <?php if (isset($error) && $error): ?>
                <div style="background: #f8d7da; color: #721c24; padding: 15px; border-radius: 10px; margin-bottom: 20px;"><?php echo $error; ?></div>
            <?php endif; ?>
            <?php if (empty($roles)): ?>
                <div class="empty-state"><div class="empty-state-icon">🎭</div><h3>Tidak ada data role</h3><p>Silakan tambahkan role baru</p></div>
            <?php else: ?>
                <table>
                    <thead><tr><th>ID Role</th><th>Nama Role</th><th>Aksi</th></tr></thead>
                    <tbody>
                        <?php foreach ($roles as $role): ?>
                        <tr>
                            <td><?php echo $role['idrole']; ?></td>
                            <td><strong><?php echo htmlspecialchars($role['nama_role']); ?></strong></td>
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-edit" onclick="window.location.href='updaterole.php?id=<?php echo $role['idrole']; ?>'">Edit</button>
                                    <button class="btn-delete" onclick="confirmDelete(<?php echo $role['idrole']; ?>)">Delete</button>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
    ...
    <script>
        function confirmDelete(id) {
            if (confirm('Apakah Anda yakin ingin menghapus role ini?')) {
                window.location.href = 'deleterole.php?id=' + id;
            }
        }
    </script>
</body>
