...
try {
    ...
    // Ambil data user dengan role kasir (role_id = 3)
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!isset($_POST['items']) || empty($_POST['items'])) {
            ...
            try {
                ...
                $sql = "INSERT INTO penjualan (subtotal_nilai, ppn, total_nilai, iduser, idmargin_penjualan) 
                        VALUES (0, :ppn, 0, :iduser, :idmargin)";
                ...
                foreach ($_POST['items'] as $item) {
                    if (!empty($item['idbarang']) && !empty($item['jumlah']) && $item['jumlah'] > 0) {
                        ...
                        $stmt_detail->bindParam(':idpenjualan', $idpenjualan);
                        $stmt_detail->bindParam(':idbarang', $item['idbarang']);
                        $stmt_detail->execute();
                    }
                }
                
                // Panggil stored procedure untuk hitung total
                $stmt = $conn->prepare("CALL hitung_total_penjualan(:id)");
                $stmt->bindParam(':id', $idpenjualan);
                $stmt->execute();
...
<head>
    ...
    <title>Tambah Penjualan - Mitra Jaya Supermarket</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-card">
            <h2 class="page-title">➕ Form Transaksi Penjualan</h2>
            ...
            <form method="POST" id="formPenjualan">
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>PPN (Rp) *</label>
                        <input type="number" name="ppn" id="ppn" value="0" min="0" required>
                    </div>
                </div>
                ...
                <table class="item-table" id="itemTable">
                    ...
                    <tbody id="itemBody">
                        <tr>
                            <td>
                                <select name="items[0][idbarang]" class="item-select" onchange="updatePrice(this, 0)" required>
                            ...
                            <td>
                                <input type="number" name="items[0][harga_satuan]" class="harga-input" 
                                       id="harga_0" readonly value="0">
                            </td>
                            <td>
                                <input type="number" name="items[0][jumlah]" class="jumlah-input" 
                                       id="jumlah_0" min="1" value="1" onchange="calculateSubtotal(0)" required>
                            </td>
                            <td>
                                <input type="number" class="subtotal-input" id="subtotal_0" readonly value="0">
                            </td>
                            <td>
