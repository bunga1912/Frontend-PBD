...
if ($id <= 0) {
    $_SESSION['delete_error'] = "ID penerimaan tidak valid!";
...
try {
    ...
    // Cek apakah data penerimaan ada
    $stmt = $conn->prepare("SELECT idpenerimaan FROM penerimaan WHERE idpenerimaan = :id");
    ...
    try {
        // Hapus detail penerimaan terlebih dahulu (karena foreign key)
        ...
        $_SESSION['delete_success'] = "Data penerimaan #$id berhasil dihapus!";
        
    } catch(Exception $e) {
        // Rollback jika ada error
        $conn->rollBack();
        $_SESSION['delete_error'] = "Gagal menghapus data penerimaan: " . $e->getMessage();
    }
