...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($jenis) || empty($nama) || empty($idsatuan) || empty($harga)) {
            ...
            // Update ke database
            $stmt = $conn->prepare("UPDATE barang SET jenis = ?, nama = ?, idsatuan = ?, status = ?, harga = ? WHERE idbarang = ?");
            $stmt->execute([$jenis, $nama, $idsatuan, $status, $harga, $id]);
...
<head>
    ...
    <title>Edit Barang - Mitra Jaya Supermarket</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="form-card">
            <h2 class="page-title">✏️ Edit Data Barang</h2>

            <div class="info-box">
                <strong>📦 ID Barang: <?php echo $barang['idbarang']; ?></strong>
                Mengubah data: <?php echo htmlspecialchars($barang['nama']); ?>
            </div>
            ...
            <form method="POST" action="">
                <div class="form-group">
                    ...
                    <input type="text" id="jenis" name="jenis" maxlength="1" required 
                           value="<?php echo htmlspecialchars($barang['jenis']); ?>"
                    ...
                    <select id="idsatuan" name="idsatuan" required>
                        ...
                        <?php foreach ($satuans as $satuan): ?>
                            <option value="<?php echo $satuan['idsatuan']; ?>"
                                <?php echo ($satuan['idsatuan'] == $barang['idsatuan']) ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" required>
                        ...
                        <option value="0" <?php echo ($barang['status'] == 0) ? 'selected' : ''; ?>>Nonaktif</option>
                    </select>
                </div>
